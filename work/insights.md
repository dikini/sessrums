...
- **Macro Path Resolution:** When generating code within a procedural macro that needs to reference items from the crate where the macro is *defined* (especially when testing within the same workspace), using absolute paths like `::crate_name::path::to::Item` is more reliable than `crate::path::to::Item` or `$crate::path::to::Item`. `$crate` refers to the macro's own crate, and `crate::` can be ambiguous depending on the invocation context.
- **Macro Invocation Scope:** Types and items generated by a procedural macro are inserted at the macro's invocation site. If these generated items need to be accessed by subsequent code within the same scope (e.g., in a test function), the macro must be invoked at a level where the generated items are visible (e.g., module level instead of inside the function).