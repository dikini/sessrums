...
// Define test message types that wrap the actual message type
// This avoids the orphan rule violation by not implementing traits directly for Server
#[derive(Clone)]
struct TestMsg;
...
fn test_choice_projection() {
    ...
    let client_local = project_for_role::<Client, TestMsg>(global.clone());
    ...
    let server_local = project_for_role::<Server, TestMsg>(global.clone());
...
#[test]
fn test_uninvolved_role_choice_projection() {
    // Create a protocol with choice where Client and Server interact:
    // choice at Client { Client -> Server: Msg1; End } or { Client -> Server: Msg2; End }
    let global = GlobalInteraction::choice(
        "client",
        vec![
    ...
    // Verify client gets a Select with Send in both branches
    match client_local {
        LocalProtocol::Select { branches, .. } => {
            assert_eq!(branches.len(), 2);
            
            // Both branches should be Send -> End
    ...
    let server_local = project_for_role::<Server, TestMsg>(global);
    
    // Verify server gets an Offer with Receive in both branches
    match server_local {
        LocalProtocol::Offer { decider, branches, .. } => {
            assert_eq!(decider.name(), "client");
            assert_eq!(branches.len(), 2);
            
            // Both branches should be Receive -> End
            for (_, cont) in branches {
                match *cont {
                    LocalProtocol::Receive { from, cont, .. } => {
                        assert_eq!(from.name(), "client");
